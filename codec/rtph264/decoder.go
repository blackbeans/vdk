...
func (d *Decoder) Read() (rtp.Header, [][]byte, error) {
	...
	case NALUTypeNonIDR, NALUTypeDataPartitionA, NALUTypeDataPartitionB,
		...
		NALUTypeReserved23:
		return pkt.Header, [][]byte{payload}, nil

	case NALUTypeFuA:
		nalus, err := d.readFragmented(payload)
		return pkt.Header, [][]byte{nalus}, err

	case NALUTypeStapA, NALUTypeStapB, NALUTypeMtap16, NALUTypeMtap24, NALUTypeFuB:
		return pkt.Header, nil, fmt.Errorf("NALU type not supported (%d)", typ)
	}

	return pkt.Header, nil, fmt.Errorf("invalid NALU type (%d)", typ)
}
